...
<head>
    ...
    <title>My Profile • Admin • DarDark</title>
            ...
            <section class="mt-6 grid grid-cols-1 gap-6 lg:grid-cols-3">
                <div class="lg:col-span-2 rounded-3xl border border-white/10 bg-white/[0.03] p-6 backdrop-blur">
                    <form id="profileForm" class="grid grid-cols-1 gap-4 md:grid-cols-2" method="POST"
                        ...
                        <p id="noChangeMsg" class="hidden text-sm text-red-300 md:col-span-2">
                        ...
                        <div>
                            ...
                            <input type="password" name="old_password" placeholder="Leave empty to keep password"
                                   class="w-full rounded-2xl border border-white/10 bg-black/30 px-4 py-3 text-sm text-white placeholder:text-white/40 outline-none transition focus:border-rose-500/40 focus:ring-4 focus:ring-rose-500/10" />
                        </div>
                        ...
                        <div class="md:col-span-2">
                            <label class="mb-2 block text-sm font-semibold text-white/90">Confirm new password</label>
                ...
                <div class="rounded-3xl border border-white/10 bg-white/[0.03] p-6 backdrop-blur">
                    <div class="flex items-start gap-4">
                        ...
                        <div>
                            <div class="text-lg font-bold tracking-tight"><?= h($userName) ?></div>
                            <div class="mt-1 text-sm text-white/60"><?= h($userEmail) ?></div>
...
submitBtn.addEventListener('click', (e) => {
    let changed = false;
    inputs.forEach(input => { if (input.value !== initialValues[input.name]) changed = true; });
    if (!changed) { e.preventDefault(); msg.classList.remove('hidden'); }
