...
<head>
    ...
    <title>My Profile • Traveler • DarDark</title>
...
<body class="min-h-screen bg-zinc-950 text-zinc-100">
    <!-- Background glow like dashboard -->
                ...
                <section class="mt-6 grid grid-cols-1 gap-6 lg:grid-cols-3">
                    ...
                    <div class="lg:col-span-2 rounded-3xl border border-white/10 bg-white/[0.03] p-6 backdrop-blur">
                        <div class="flex items-center justify-between gap-3 flex-wrap">
                            <div>
                                <div class="text-sm text-white/70">Account details</div>
                                <div class="mt-1 text-lg font-bold">Profile settings</div>
                            </div>
                        ...
                        <form id="profileForm" class="mt-6 grid grid-cols-1 gap-4 md:grid-cols-2" method="POST"
                              action="/airbnb-php-oop/actions/updateProfile.php">

                            <p id="noChangeMsg" class="hidden md:col-span-2 rounded-2xl border border-red-500/30 bg-red-500/10 px-4 py-3 text-sm text-red-200">
                            ...
                            <div class="md:col-span-2">
                                ...
                                <input name="name" value="<?= h($userName) ?>" required
                            ...
                            <div class="md:col-span-2">
                                ...
                                <input type="email" name="email" value="<?= h($userEmail) ?>" required
                                       class="w-full rounded-2xl border border-white/10 bg-black/30 px-4 py-3 text-sm text-white placeholder:text-white/40 outline-none transition focus:border-rose-500/40 focus:ring-4 focus:ring-rose-500/10" />
                            </div>
                    ...
                    <div class="rounded-3xl border border-white/10 bg-white/[0.03] p-6 backdrop-blur">
                        <div class="flex items-start gap-4">
                            <div class="h-14 w-14 rounded-2xl bg-gradient-to-br from-rose-500 to-red-600 shadow-lg shadow-rose-500/20"></div>
                            <div class="min-w-0">
                                <div class="text-lg font-bold tracking-tight truncate"><?= h($userName) ?></div>
                                <div class="mt-1 text-sm text-white/60 truncate"><?= h($userEmail) ?></div>
                                <div class="mt-2 inline-flex rounded-full border border-white/10 bg-white/5 px-3 py-1 text-xs font-semibold text-white/70">
                                    Traveler
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="mt-6 grid grid-cols-1 gap-3">
                            <div class="rounded-2xl border border-white/10 bg-black/20 p-4">
                                ...
                                <div class="mt-1 text-sm font-semibold"><?= h($memberSince) ?></div>
                            </div>
    ...
    <script>
        ...
        submitBtn.addEventListener('click', (e) => {
            ...
            inputs.forEach(input => {
                if (input.value !== initialValues[input.name]) changed = true;
            });
