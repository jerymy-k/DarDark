...
<body class="min-h-screen bg-zinc-950 text-zinc-100">
    ...
    <main class="relative mx-auto flex min-h-screen max-w-6xl items-center justify-center px-4 py-10">
        <div class="grid w-full grid-cols-1 items-stretch gap-8 md:grid-cols-2">
            ...
            <section
                ...
                <div>
                    ...
                    <p class="mt-6 text-sm leading-6 text-white/70">
                        DarDark is a short-term rental platform that connects travelers with trusted hosts.
                        Discover unique stays, manage bookings, and communicate easily — all in one place.
                    </p>
                    ...
                    <div class="mt-8 space-y-4">
                        <div class="flex items-start gap-3">
                            ...
                            <div>
                                <p class="font-semibold">Browse unique rentals</p>
                                <p class="text-sm text-white/60">
                                    Explore homes and apartments with detailed information and photos.
                                </p>
                            </div>
                        </div>
                        ...
                        <div class="flex items-start gap-3">
                            ...
                            <div>
                                ...
                                <p class="text-sm text-white/60">
                                    Check availability and book stays in just a few clicks.
                                </p>
                            </div>
                        </div>
                        ...
                        <div class="flex items-start gap-3">
                            <span
                                class="mt-1 inline-flex h-6 w-6 items-center justify-center rounded-lg bg-white/5 text-white/80">💬</span>
                            <div>
                                <p class="font-semibold">Direct communication</p>
                                <p class="text-sm text-white/60">
                                    Chat securely with hosts before and after booking.
                                </p>
                            </div>
                        </div>
                    </div>
                ...
                <p class="text-xs text-white/50">
                    Join thousands of travelers and hosts using DarDark to manage their stays with confidence.
                </p>
            </section>
            ...
            <section class="rounded-3xl border border-white/10 bg-white/[0.03] p-6 backdrop-blur md:p-8">
                ...
                <form class="space-y-5" method="POST" action="/airbnb-php-oop/actions/signupAction.php">
                    <!-- Full Name -->
                    <div>
                        <label for="name" class="mb-2 block text-sm font-semibold text-white/90">
                            Full name
                        </label>
                        <input id="name" name="name" type="text" placeholder="e.g. Ahmed El Amrani" required class="w-full rounded-2xl border border-white/10 bg-black/30 px-4 py-3 text-sm text-white placeholder:text-white/40 outline-none transition
               focus:border-rose-500/40 focus:ring-4 focus:ring-rose-500/10" />
                        <p class="mt-2 hidden text-xs text-red-200" id="errName">
                            Name is required.
                        </p>
                    </div>
                    ...
                    <!-- Email -->
                    <div>
                        <label for="email" class="mb-2 block text-sm font-semibold text-white/90">
                            Email
                        </label>
               ...
               focus:border-rose-500/40 focus:ring-4 focus:ring-rose-500/10" />
                        <p class="mt-2 hidden text-xs text-red-200" id="errEmail">
                    ...
                    <!-- Password -->
                    <div>
                        <label for="password" class="mb-2 block text-sm font-semibold text-white/90">
                            Password
                        </label>

                        <div class="relative">
                            <input id="password" name="password" type="password" placeholder="Create a strong password"
                                required minlength="6" class="w-full rounded-2xl border border-white/10 bg-black/30 px-4 py-3 pr-12 text-sm text-white placeholder:text-white/40 outline-none transition
                 focus:border-rose-500/40 focus:ring-4 focus:ring-rose-500/10" />
                 ...
                 hover:bg-white/10 transition">
                    ...
                    <!-- Confirm Password -->
                    <div>
                        <label for="confirm_password" class="mb-2 block text-sm font-semibold text-white/90">
                            Confirm password
                        </label>

                        <input id="confirm_password" name="confirm_password" type="password"
                            placeholder="Re-enter your password" required class="w-full rounded-2xl border border-white/10 bg-black/30 px-4 py-3 text-sm text-white placeholder:text-white/40 outline-none transition
               focus:border-rose-500/40 focus:ring-4 focus:ring-rose-500/10" />
                    </div>

                    <!-- Role -->
                    <div>
                        <label for="role" class="mb-2 block text-sm font-semibold text-white/90">
                            Role
                        </label>
                        <select id="role" name="role" required class="w-full appearance-none rounded-2xl border border-white/10 bg-black/30 px-4 py-3 text-sm text-white outline-none transition
               focus:border-rose-500/40 focus:ring-4 focus:ring-rose-500/10">
    ...
    <script>
        // Toggle password visibility (simple UX improvement)
        ...
        // Tiny password hint (optional)
        pwd.addEventListener('input', () => {
            const v = pwd.value.trim();
            if (!v) { hint.textContent = ''; return; }
            if (v.length < 6) hint.textContent = 'Weak';
            else if (v.length < 10) hint.textContent = 'Good';
            else hint.textContent = 'Strong';
        });
    </script>
</body>
